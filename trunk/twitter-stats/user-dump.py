...
#Adapted from: https://github.com/kylemanna/social-utils/blob/master/twitter/user-dump-followers.py

def dump_followers(api):
    print "Dumping followers"
    followers = []
    cursor= tweepy.Cursor(api.followers, count=500).items()
    for u in cursor:
	print u.name
        followers.append({
            'id':u.id,
...
def dump_friends(api):
    ...
    friends = []
    for u in tweepy.Cursor(api.friends, count=300).items():
        friends.append({
            'id':u.id,
            'screen_name':u.screen_name,
	    'description':u.description,
